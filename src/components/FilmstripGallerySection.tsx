...
const photos: Photo[] = [
  ...
  {
    id: 2,
    src: getPhotoSrc('speaking2.webp'),
    caption: 'Public Speaking',
    subcaption: 'EduACS Seminar Series — 2024',
    category: 'university',
    year: '2024',
  },
  {
...
const categoryConfig: Record<Category, { label: string; color: string; glow: string }> = {
  event:      { label: 'Event',      color: 'text-blue-300 border-blue-500/40 bg-blue-500/10',    glow: 'rgba(30,107,196,0.4)' },
  university: { label: 'University', color: 'text-amber-300 border-amber-500/40 bg-amber-500/10', glow: 'rgba(200,160,60,0.4)' },
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/4 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl -translate-y-1/2" />
          ...
          {filters.map(f => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 ${
                filter === f.value
                  ? 'bg-blue-500 text-black border-blue-500 shadow-lg shadow-blue-500/30'
      ...
      <div className="relative">
        ...
        <div className="flex gap-0 overflow-hidden h-6 bg-[#0a1628] border-y border-white/5">
          {Array.from({ length: 40 }).map((_, i) => (
            <div key={i} className="flex-shrink-0 w-10 flex items-center justify-center">
              <div className="w-5 h-3 rounded-sm bg-[#060d1a] border border-white/10" />
            </div>
          ))}
        </div>
        ...
        <div
          className="bg-[#0a1628] border-y border-white/5 py-6"
        >
          <div
            ref={stripRef}
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            >
              {/* Film frame border */}
              <div className="bg-[#0a1628] p-4 rounded-lg border border-white/10">
                {/* Top sprockets */}
                <div className="flex justify-between mb-3 px-1">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="w-8 h-4 rounded-sm bg-[#060d1a] border border-white/15" />
                  ))}
                </div>

                {/* Image */}
                <div className="aspect-video bg-[#060d1a] rounded overflow-hidden relative">
                  <img
                    src={lightbox.src}
              ...
              <button
                onClick={() => setLightbox(null)}
                className="absolute -top-3 -right-3 p-2 rounded-full bg-[#060d1a] border border-white/20 text-gray-400 hover:text-white transition-all"
                aria-label="Close"
              >
...
function FilmFrame({ photo, index, isHovered, onHover, onLeave, onClick }: FilmFrameProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Frame number strip */}
        <div className="bg-[#0f1e35] px-2 py-1 flex items-center justify-between">
        ...
        {/* Photo */}
        <div
          className="relative overflow-hidden bg-[#060d1a]"
          style={{ height: '180px' }}
        >
          <img
            src={photo.src}
        ...
        {/* Category strip */}
        <div className={`px-3 py-1.5 flex items-center justify-between bg-[#0f1e35]`}>
